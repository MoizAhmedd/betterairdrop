...
/// A plain folder standing in for the Trash (tests, and `BETTERAIRDROP_TRASH_DIR` for dry-runs on real data).
...
public enum Trash {
    /// `BETTERAIRDROP_TRASH_DIR` swaps the real Trash for a folder.
    public static func `default`() -> any Trasher {
        if let p = ProcessInfo.processInfo.environment["BETTERAIRDROP_TRASH_DIR"] { return DirectoryTrash(URL(fileURLWithPath: p)) }
        return SystemTrash()
    }
}
