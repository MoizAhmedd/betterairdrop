...
/// `~/.config/betterairdrop/config.toml`. Every key is optional.
public struct Config: Sendable, Equatable {
    ...
    public static var defaultPath: URL {
        ...
        if let p = env["BETTERAIRDROP_CONFIG"] { return URL(fileURLWithPath: p) }
        _ = LegacyMigration.once
        return configBase.appendingPathComponent("betterairdrop/config.toml")
    }

    static var configBase: URL {
    ...
    /// Parses the TOML subset betterairdrop uses: `key = value` pairs and one level of `[tables]`.
    ...
    /// Sets a dotted key (`watch.enabled`). Used by the parser and by `betterairdrop config set`.
