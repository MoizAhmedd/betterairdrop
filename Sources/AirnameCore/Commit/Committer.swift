...
///   4. set the `dev.airname.done` marker
...
public final class Committer {
    ...
    func commitOne(_ p: Proposal, target planned: URL, batch: String) throws -> Outcome {
        ...
        do {
            ...
            if p.action == .convert {
                let t = dir.appendingPathComponent(".airname-\(UUID().uuidString).jpg.tmp")
                temp = t
    ...
    /// Removes leftover `.airname-*.tmp` files older than a minute (from a crash mid-conversion).
    public static func sweepTemporaries(in dir: URL, olderThan age: TimeInterval = 60) {
        ...
        for u in items where u.lastPathComponent.hasPrefix(".airname-") && u.pathExtension == "tmp" {
