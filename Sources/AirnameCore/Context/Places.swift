...
public final class Places: @unchecked Sendable {
    ...
    public enum Error: Swift.Error, CustomStringConvertible {
        ...
        public var description: String {
            ...
            case .notFound: "cities table not found (set AIRNAME_CITIES or reinstall)"
            }
        }
    }
    ...
    /// Loads the table: `$AIRNAME_CITIES`, then `<prefix>/share/airname/cities.bin`, then the SwiftPM resource bundle.
    public static func load() throws -> Places {
        var candidates: [URL] = []
        if let env = ProcessInfo.processInfo.environment["AIRNAME_CITIES"] { candidates.append(URL(fileURLWithPath: env)) }
        let exe = URL(fileURLWithPath: CommandLine.arguments[0]).resolvingSymlinksInPath().deletingLastPathComponent()
        candidates.append(exe.appendingPathComponent("../share/airname/cities.bin").standardized)
