...
struct Undo: ParsableCommand {
    ...
    @Option(help: "Undo a specific batch (IDs are shown by `betterairdrop log`).")
    var batch: String?

    @Argument(help: "Undo the change that produced (or consumed) this file.", completion: .file())
    var file: String?

    @Flag(help: "Undo even if the output was modified since betterairdrop wrote it.")
    var force = false
