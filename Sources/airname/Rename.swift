...
struct Rename: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        discussion: "Nothing changes on disk with --dry-run. Without it, HEIC files are converted to JPEG and the original goes to the Trash (see `airname undo`)."
    )
    ...
    func printOutcomes(_ outcomes: [Committer.Outcome], batch: String) {
        ...
        if n > 0 { print("Renamed \(n) file\(n == 1 ? "" : "s"). Undo with: airname undo --batch \(batch)") }
    }
