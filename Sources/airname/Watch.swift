...
struct Watch: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        undo: `airname undo` (last batch) or `airname undo --batch ID`.
        --foreground runs in this Terminal window, which already has access to Downloads. The background
        LaunchAgent (`airname install`) comes later.
        """
    )
    ...
    func run() throws {
        guard foreground || once else {
            throw ValidationError("The background agent isn't available yet. Run `airname watch --foreground` in Terminal.")
        }
        ...
        guard let lock = ProcessLock() else {
            throw ValidationError("another airname watcher is already running")
        }
        ...
        watcher.onBatch = { b in
            ...
            print("\(summary). Undo: airname undo --batch \(b.id)\(tokens.isEmpty ? "" : String(format: "  (Claude: %d photo%@, about $%.4f)", tokens.count, tokens.count == 1 ? "" : "s", cost))")
            if notify { Notifier.post(title: "airname", message: "\(summary) · undo: airname undo") }
        }

        let backendDesc = namer.map { Backends.isCloud($0) ? "claude (Apple Vision if it fails)" : $0.id } ?? "none"
        print("airname: watching \(tilde(folder.path)) for \(options.airdropOnly ? "AirDrop arrivals" : "new photos")"
        ...
        source.setEventHandler {
            ...
            stop.set()
            FileHandle.standardError.write(Data("\nairname: stopping after the current batch (Ctrl-C again to quit now)\n".utf8))
        }
        source.resume()
        watcher.run(shouldStop: { stop.isSet })
        print("airname: stopped.")
    }
