...
/// `swift test`:
///
///     BETTERAIRDROP_E2E=1 swift test --filter E2E      (or scripts/e2e.sh)
///
...
enum E2E {
    static let enabled = ProcessInfo.processInfo.environment["BETTERAIRDROP_E2E"] == "1"
    ...
    final class Report: @unchecked Sendable {
        ...
        func html() -> String {
            ...
            var h = """
            <!doctype html><meta charset="utf-8"><title>betterairdrop e2e report</title>
            <style>
            ...
            <h1>betterairdrop e2e: Claude vs Vision</h1>
...
@Suite(.enabled(if: E2E.enabled, "set BETTERAIRDROP_E2E=1 to run the live Claude e2e tests"), .serialized)
struct E2ENamingTests {
    ...
    @Test func fullPipeline() throws {
        try #require(E2E.credentialAvailable, "no Anthropic credential: see `betterairdrop auth status`")
